...
class New1Spider(scrapy.Spider):
    """
    存在问题:
    采用Requestback方法最终获取图书价格时未能完全请求保存
    """
    name = 'new1'
    ...
    def parse(self, response):
        ...
        dt_list = response.xpath('//*[@id="booksort"]/div[2]/dl/dt[1]')  # 测试,取第一个

        for i in dt_list:

            item['big_name'] = i.xpath('.//a/text()').extract_first()
            # small_list = i.xpath('./following-sibling::*[1]/em')
            small_list = i.xpath('./following-sibling::*[1]/em[1]')  # 测试,取第一个
